...
export default function ProductHero() {
  const [activeImage, setActiveImage] = useState('https://images.unsplash.com/photo-1590253232292-5c0c4c04e428?auto=format&fit=crop&q=80&w=1200');
            ...
            <div className="flex gap-4">
              {thumbnails.map((thumb, idx) => (
                ...
                >
                  ...
                  <img src={thumb} alt={`Thumbnail ${idx + 1}`} className="object-contain w-full h-full p-2" />
                </button>
              ))}
            </div>
          </motion.div>
          ...
          >
            <div>
              <div className="flex items-center gap-3 mb-3">
                <span className="text-cyan-400 font-semibold uppercase tracking-wider text-sm">NVIDIA</span>
                <span className="w-1.5 h-1.5 rounded-full bg-gray-600" />
                <span className="text-green-400 flex items-center gap-1 text-sm"><Activity className="w-4 h-4" /> In Stock</span>
              </div>
              ...
              <div className="flex items-center gap-6 text-sm mb-6">
                <div className="flex items-center gap-2">
                  <div className="flex text-yellow-400">
                    {[...Array(5)].map((_, i) => <Star key={i} className={`w-5 h-5 ${i < 4 ? 'fill-current' : 'text-gray-600'}`} />)}
                  </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 relative z-10">
                <div>
                  ...
                  <div className="flex items-baseline gap-2">
                    <span className="text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400">$399</span>
                ...
                <div className="flex flex-col items-center">
                  <div className="relative w-16 h-16 rounded-full flex items-center justify-center bg-gray-800 border border-gray-700 shadow-[0_0_15px_rgba(0,229,255,0.3)] mb-2">
                    <svg className="absolute inset-0 w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                      ...
                      <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="#00e5ff" strokeWidth="3" strokeDasharray="93, 100" className="animate-[spin_2s_ease-out]" />
                    </svg>
                    <span className="text-xl font-bold text-cyan-400">93</span>
                  </div>
                  <span className="text-xs font-semibold text-gray-400 uppercase tracking-wide">AI Score</span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-3 gap-4 mt-2">
              {[
                ...
                <div key={i} className="bg-white/5 border border-white/5 rounded-2xl p-4 flex flex-col items-center justify-center gap-2 text-center">
